...
if test -z "$BUILD_LOG_PATH"
    set -g BUILD_LOG_PATH "$BUILD_DIR/swift-compile.xcodebuild.log"
end
...
function refresh_build_settings
    set -g BUILD_SETTINGS_CACHE (xcodebuild $PROJECT_OPTION "$PROJECT_FILE" -scheme "$SCHEME_NAME" -configuration "$CONFIGURATION" -showBuildSettings 2>/dev/null)
end
...
function resolve_product_path
    ...
    set -l build_target_dir (read_build_setting TARGET_BUILD_DIR); or return 1
...
function print_summary
    if test "$PROJECT_KIND" = xcode
        refresh_build_settings >/dev/null 2>&1
    end

    set -l product_path (resolve_product_path 2>/dev/null)
